...
func NewRegistrar(client Client, service Service, logger logrus.FieldLogger) *Registrar {
	return &Registrar{
		client:  client,
		service: service,
		logger:  logger.WithFields(logrus.Fields{"key": service.Key, "value": service.Value}),
	}
}
...
func (r *Registrar) Register() {
	...
	if r.service.TTL != nil {
		r.logger.Info("action", "register", "lease", r.client.LeaseID())
	} else {
		r.logger.Info("action", "register")
	}
}
...
func (r *Registrar) Deregister() {
	if err := r.client.Deregister(r.service); err != nil {
		...
		r.logger.Info("action", "deregister")
	}
